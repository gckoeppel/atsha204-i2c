...
static int hwcrypt_device_open(struct inode* inode, struct file* filp)
{
        ...
        /* Ensure that only one process has access to our
...
static ssize_t hwcrypt_device_read(struct file* filp,
                                   char __user *buffer,
                                   size_t length,
                                   loff_t* offset)
{

        /* The default from 'cat' is to issue multiple reads until the FIFO
           is depleted one_shot avoids that */
...
/* The file_operation scructure tells the kernel which device
   operations are handled.
   For a list of available file operations, see
...
static ssize_t sys_serialnum(struct device* dev,
...
/* Declare the sysfs entries.
   The macros create instances of dev_attr_serialnumber */
...
static int __init hwcrypt_module_init(void)
{
        ...
        /* We can either tie our device to a bus
           (existing, or one that we create)
         * or use a "virtual" device class. For this example,
 ...
 /* With a class, the easiest way to instantiate a device is to call
    device_create() */
        hwcrypt_device = device_create(hwcrypt_class, NULL,
                                       MKDEV(hwcrypt_major, 0),
                                       NULL,
